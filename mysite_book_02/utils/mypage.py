...
class Page():
    ...
    def __init__(self, page_num, total_count, url, per_page=10, max_page=11):
        """
    ...
    def page_html(self):
        ...
        html_str_list = []
        # 添加首页
        html_str_list.append('<li><a href="{}?page=1">首页</a></li>'.format(self.url))
        ...
        if self.page_num <= 1:
            ...
            # 添加上一页标签
            html_str_list.append('<li><a href="{0}?page={1}" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>'.format(self.url, self.page_num-1))

        for i in range(self.page_start, self.page_end+1):
            # 如果是当前页就加一个active样式类
            if i == self.page_num:
                tmp = '<li class="active"><a href="{0}?page={1}">{1}</a></li>'.format(self.url, i)
            else:
                tmp = '<li><a href="{0}?page={1}">{1}</a></li>'.format(self.url, i)
        ...
        if self.page_num >= self.total_page:
            ...
            # 添加下一页标签
            html_str_list.append('<li><a href="{}?page={}" aria-label="Previous"><span aria-hidden="true">&raquo;</span></a></li>'.format(self.url, self.page_num+1))
        # 添加尾页
        html_str_list.append('<li><a href="{0}?page={1}">尾页:{1}</a></li>'.format(self.url, self.total_page))
